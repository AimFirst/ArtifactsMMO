...
class PerformTaskTarget extends Target {
  PerformTaskTarget({required super.parentTarget, required super.characterLog});
  ...
  TargetProcessResult update(
    ...
    if (progress.finished) {
      return TargetProcessResult(
          progress: progress,
          action: null,
          description: 'Finished task ${character.taskProgress?.taskCode}',
    ...
    if (character.taskProgress?.taskCode != null) {
      switch (character.taskProgress!.taskType) {
        ...
        case TaskType.monsters:
          return FightMonsterTaskTarget(parentTarget: this, characterLog: characterLog,).update(
              character: character,
  ...
  Progress getProgress({
    ...
    return Progress(
      current: character.taskProgress?.progress.toDouble() ?? 0,
