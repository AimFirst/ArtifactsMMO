...
abstract class FightTarget extends Target {
  ...
  TargetProcessResult update({required Character character, required BoardState boardState, required ArtifactsClient artifactsClient}) {
    ...
    if (progress.finished) {
      return TargetProcessResult(progress: progress, action: null, description: 'Fight target done.', imageUrl: 'https://artifactsmmo.com/images/items/iron_sword.png');
    }

    final heal = HealTarget().update(character: character, boardState: boardState, artifactsClient: artifactsClient);
    if (!heal.progress.finished) {
      return heal;
    }

    List<Monster> targetMonsters = getTargetMonster(character: character, boardState: boardState);
    for (final monster in targetMonsters) {
      ...
      if (canDefeatMonster(character: character, monster: monster)) {
        ...
        return TargetProcessResult(progress: progress, action: artifactsClient.fight(action: ActionFight(characterName: character.name)), description: 'Fighting $monster', imageUrl: 'https://artifactsmmo.com/images/items/christmas_star.png');
      }
    }
    return FightLevelTarget(level: character.overall.level + 1).update(character: character, boardState: boardState, artifactsClient: artifactsClient);
  }
