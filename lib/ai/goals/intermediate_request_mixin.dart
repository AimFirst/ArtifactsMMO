...
mixin class IntermediateRequestMixin {
  Map<String, SimpleItemSchema> neededRequestItems(TeamBrainProvider teamBrainProvider) {
    ...
    for (final request in teamBrainProvider.openRequests) {
      ...
      final currentNeed = itemsNeeded.putIfAbsent(requestedItem.code, () => SimpleItemSchemaBuilder().fromCodeAndQuantity(requestedItem.code, 0));
      itemsNeeded[requestedItem.code] = SimpleItemSchemaBuilder().fromCodeAndQuantity(requestedItem.code, currentNeed.quantity + requestedItem.quantity);
    }
    return itemsNeeded;
  }

  List<SimpleItemSchema> remainingNeededItems(TeamBrainProvider teamBrainProvider, BankProvider bankProvider) {
    ...
    for (final neededItem in neededItems.values) {
      ...
      if (bankCount < neededItem.quantity) {
        itemsNeeded.add(SimpleItemSchemaBuilder().fromCodeAndQuantity(neededItem.code, neededItem.quantity - bankCount));
      }
    }
    return itemsNeeded;
  }
