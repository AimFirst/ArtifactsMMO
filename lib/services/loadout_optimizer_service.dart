...
class LoadoutOptimizerService {
  ...
  LoadoutOptimizerService(
      this._combatService, this._worldDataProvider);

  String _generateGearEvaluationContextKey(GearEvaluationContext gearContext) {
    return gearContext.toString();
  }

  String _generateCharacterSkillsAndGearCacheKey(
      CharacterSchema character, String skillName, List<ItemSchema?> gearOptions) {
    // Only include stats relevant to the combat calculation.
    final relevantStatsKey =
        character.skills[skillName]?.level.toString();
    final gearOptionsKey = gearOptions.where((item) => item != null).map((item) => item!.code).toList()..sort((a,b) => a.compareTo(b))..join(',');
    return "$relevantStatsKey|$gearOptionsKey";
  }

  EquipmentLoadoutResult _getLoadoutResult(GearEvaluationContext gearContext, CharacterSchema character, EquipmentLoadout loadout) {
    final contextKey = _generateGearEvaluationContextKey(gearContext);
    final characterKey = _generateCharacterSkillsAndGearCacheKey(character, gearContext.taskType, loadout.items);
    ...
    CharacterSchema tempCharacter =
    character.copyWithEquippedItems(loadout.itemsBySlot, _worldDataProvider);

    if (gearContext.taskType == CharacterExtensions.overallLevelSkillName && gearContext.targetMonster != null) {
  ...
  EquipmentLoadoutResult _bestOption(CharacterSchema characterSchema, GearEvaluationContext gearContext, EquipmentLoadout loadout, Map<ItemSlot, List<ItemSchema?>> gearOptions, int index) {
    final contextKey = _generateGearEvaluationContextKey(gearContext);
    final characterKey = _generateCharacterSkillsAndGearCacheKey(characterSchema, gearContext.taskType, gearOptions.values.expand((items) => items).toList());
    ...
    for (final item in (gearOptions[itemSlot] ?? <ItemSchema?>[])) {
      ...
      if (index >= itemSlots.length - 1) {
        results.add(_getLoadoutResult(gearContext, characterSchema, newLoadout));
      } else {
        results.add(_bestOption(characterSchema, gearContext, newLoadout, gearOptions, index + 1));
      }
    }

    // Compare the combinations to get the best results
    results.sort((a, b) {
      if (a is CombatEquipmentLoadoutResult && b is CombatEquipmentLoadoutResult) {
        ...
        return b.loadout.items.where((item) => item == null).length.compareTo(a.loadout.items.where((item) => item == null).length);
      } else if (a is SkillEquipmentLoadoutResult && b is SkillEquipmentLoadoutResult) {
        final effectEnum = EffectEnum.values.firstWhere((e) => e.name == gearContext.taskType);
    ...
    final topLevel = _bestResults.putIfAbsent(contextKey, () => <String, EquipmentLoadoutResult>{});
    topLevel[characterKey] = bestResult;

    return bestResult;
  }

  EquipmentLoadoutResult bestLoadout(CharacterSchema character, GearEvaluationContext gearContext, List<ItemSchema?> allItemsToConsider) {
    final List<ItemSchema?> itemsThisCharacterCanUse = allItemsToConsider.where((item) => item != null && character.canUseItem(item)).toList();
    Map<ItemSlot, List<ItemSchema?>> gearOptions = {};
    for (final slot in ItemSlot.values) {
      gearOptions[slot] = itemsThisCharacterCanUse.where((item) => item?.type == slot.name).cast<ItemSchema?>().toList()..add(null);
    }
    return _bestOption(character, gearContext, EquipmentLoadout(), gearOptions, 0);
  }

  EquipmentLoadoutResult bestLoadoutOfAvailableItems(CharacterSchema character, GearEvaluationContext gearContext, List<ItemSchema?> inventoryItems, List<ItemSchema?> bankItems) {
    return bestLoadout(character, gearContext, [...EquipmentLoadout.fromCharacter(character, _worldDataProvider).items, ...inventoryItems, ...bankItems]);
  }
}
