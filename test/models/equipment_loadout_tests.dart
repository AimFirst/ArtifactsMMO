...
// class MockEffectSchema extends Mock implements EffectSchema {}
//
// void main() {
...
//     MockItemSchema createMockItem({List<SimpleEffectSchema>? effects, String code = 'test_item'}) {
//       final item = MockItemSchema();
//       when(() => item.code).thenReturn(code);
//       when(() => item.effects).thenReturn(BuiltList.of(effects ?? <SimpleEffectSchema>[]));
//       return item;
//     }
//
...
//         when(() => mockCharacter.shieldSlot).thenReturn(''); // Example of an empty slot
...
//         when(() => mockWorldDataProvider.getItemByCode('sword_id')).thenReturn(weaponItem);
//         when(() => mockWorldDataProvider.getItemByCode('helmet_id')).thenReturn(helmetItem);
//         when(() => mockWorldDataProvider.getItemByCode('ring1_id')).thenReturn(ring1Item);
//         // For null slots, getItemByCode should be called and can return null
//         when(() => mockWorldDataProvider.getItemByCode(null)).thenReturn(null);
//
//
//         final loadout = EquipmentLoadout.fromCharacter(mockCharacter, mockWorldDataProvider);
//
...
//       expect(itemsList.where((item) => item == null).length, 13); // If only 3 items are set
//     });
//
//     group('effectValue', () {
...
//         final effect1 = MockEffectSchema();
//         when(() => effect1.code).thenReturn(EffectEnum.dmg.name);
//         when(() => effect1.value).thenReturn(10.0);
//
...
//         final hpEffect = MockEffectSchema();
//         when(() => hpEffect.code).thenReturn(EffectEnum.hp.name);
//         when(() => hpEffect.value).thenReturn(50.0);
//
...
//         final hpEffect1 = MockEffectSchema();
//         when(() => hpEffect1.code).thenReturn(EffectEnum.hp.name);
//         when(() => hpEffect1.value).thenReturn(30.0);
//
//         final hpEffect2 = MockEffectSchema();
//         when(() => hpEffect2.code).thenReturn(EffectEnum.hp.name);
//         when(() => hpEffect2.value).thenReturn(20.0);
//
//         // Also add a different effect to ensure it's ignored
//         final dmgEffect = MockEffectSchema();
//         when(() => dmgEffect.code).thenReturn(EffectEnum.dmg.name);
//         when(() => dmgEffect.value).thenReturn(5.0);
//
//         final amulet = createMockItem(effects: [hpEffect1, dmgEffect, hpEffect2]);
...
//       });
//
//       test('handles items with effects list containing null EffectSchema (if possible by contract)', () {
...
//         final hpEffect = MockEffectSchema();
//         when(() => hpEffect.code).thenReturn(EffectEnum.hp.name);
//         when(() => hpEffect.value).thenReturn(10.0);
//
...
//
//
//       test('complex scenario with multiple items, multiple effects, and some missing effects', () {
//         final hpEffect1 = MockEffectSchema();
//         when(() => hpEffect1.code).thenReturn(EffectEnum.hp.name);
//         when(() => hpEffect1.value).thenReturn(100.0);
//         final weapon = createMockItem(effects: [hpEffect1]);
//
//         final hpEffect2 = MockEffectSchema();
//         when(() => hpEffect2.code).thenReturn(EffectEnum.hp.name);
//         when(() => hpEffect2.value).thenReturn(50.0);
//         final dmgEffect1 = MockEffectSchema();
//         when(() => dmgEffect1.code).thenReturn(EffectEnum.dmg.name);
//         when(() => dmgEffect1.value).thenReturn(5.0);
//         final helmet = createMockItem(effects: [hpEffect2, dmgEffect1]);
//
//         final dmgEffect2 = MockEffectSchema();
//         when(() => dmgEffect2.code).thenReturn(EffectEnum.dmg.name);
//         when(() => dmgEffect2.value).thenReturn(10.0);
...
//
//         final loadout = EquipmentLoadout(
//           weapon: weapon,       // HP: 100
//           helmet: helmet,       // HP: 50, DMG: 5
//           shield: shield,       // DMG: 10
//           bodyArmor: emptyItem, // No effects
//           legArmor: nullEffectsItem, // Null effects
//           boots: null,          // No item
//         );
//
//         expect(loadout.effectValue(EffectEnum.hp), 150.0);
//         expect(loadout.effectValue(EffectEnum.dmg), 15.0);
//         expect(loadout.effectValue(EffectEnum.res_air), 0.0); // Effect not present
//       });
//     });
//   });